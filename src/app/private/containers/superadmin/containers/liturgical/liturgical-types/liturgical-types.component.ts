...
@Component({
  selector: 'app-liturgical-types',
  templateUrl: './liturgical-types.component.html',
  styleUrls: ['./liturgical-types.component.scss']
})
export class LiturgicalTypesComponent implements OnInit {
  ...
  constructor(
    ...
    private dataService: LiturgicaltypeService
  ) { }

  ngOnInit() {
    this.user = this.authService.getUserInfos();
    this.changeLanguage(this.currentLanguage);
    this.initForm({title: '', slug: '', description: ''});
    setTimeout( () => {
      this.gets(this.page);
    }, 2000);
  }
